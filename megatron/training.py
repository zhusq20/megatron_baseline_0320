    ...
    if iteration % args.log_interval == 0:
        ...
        if is_last_rank():
            '''with open(filename, "a") as file:
                if iteration == 1: 
                    file.write("new test" + str(torch.distributed.get_world_size()) + "\n\n")
                file.write(log_string + "\n\n")
            '''
            import wandb
            name = f"{args.global_batch_size=},{args.micro_batch_size=},{args.seq_length=},{args.tensor_model_parallel_size=},{args.pipeline_model_parallel_size=},{args.data_parallel_size=}"
            wandb.init(
                project="megatron octave test 18march",
                name=name,
                config={"command": vars(args)},
                )
            wandb.log(
                    {
                         "tput global(global bs*seqlen/iter_time)":(batch_size * args.seq_length) / elapsed_time_per_iteration,
                         "iteration time": elapsed_time_per_iteration,
                         "batch size": batch_size,
                         "peak mem": torch.cuda.max_memory_allocated() / (1024 * 1024 * 1024),
                         "valley mem": torch.cuda.memory_allocated() / (1024 * 1024 * 1024),
                    },
                step=iteration,
                )
