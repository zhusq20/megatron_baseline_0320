...
from puzzle.utils.perf import print_throughput
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step in range(num_total_iters):
            ...
            if torch.distributed.get_rank() == torch.distributed.get_world_size() - 1:
                print(f'epoch: {epoch}|step: {step}|ppo_ep: {ppo_ep+1}|act_loss: {actor_loss_sum/inner_iter}|cri_loss: {critic_loss_sum/inner_iter}')

            e2e_time = training_time + generation_time
            if torch.distributed.get_rank() == 0:
                print_throughput(trainer, trainer.generate_time, training_time, e2e_time)
